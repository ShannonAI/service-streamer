...
<p align="center">Speed Up Web Services of Deep Learning Applications</p>


<p align="center">
  <a href="#What is Service Streamer ?">What is Service Streamer ?</a> •
  <a href="#Functionality Highlights">Functionality Highlights</a> •
  <a href="#Installation">Installation</a> •
  <a href="#Deploy BERT Service in 5 Mintues">Deploy BERT Service in 5 Mintues</a> •
  <a href="#Introduction of API">Introduction of API</a> •
  <a href="#Benchmark Test">Benchmark Test</a> •
...
A mini-batch collects induvidual data samples and is usually adopted in deep learning models during training and inference. Thus models can fully utilize the parallel computation characterristic of GPU and speed up computing. Requests from users are usually discrete when machine learning models are deploied online. But there comes an issue that computing processors are idle when using conventional synchronous blocking message commuication mechanism. The user's waiting time will become longer when there are requests from a large number of users in a short time. 
...
<h2 align="center">Functionality Highlights</h2>
...
- :crossed_swords: **Applicability**: Easy to be plugged into web service frameworks of machine learning applications. 
...
<h2 align="center">Deploy BERT Service in 5 Mintues</h2>

We provide a step-by-step tutorial for you to deploy BERT online in 5 minutes. The service processes 1400 sentences per second.  
...
2. Second, utilize [Flask](https://github.com/pallets/flask) to pack predicting interfaces to Web service. [flask_example.py](./example/flask_example.py)
    ...
    At this time, your web server can only response 12 requests per second, see [benchmark] (# benchmark)


3. Third, encapsulate model function through `service_streamer`, and three lines of code make the prediction speed of BERT service reach 200+ sentences per second (16 times QPS).
    ...
    Run [Flask _ Example.py] (./Example/Flask _ Example.py) and test the performance with [WRK] (https://github.com/WG/WRK). 
...
4. Finally, encapsulate models through ``Streamer`` and start service workers on multi-card GPU. ``Streamer`` further accelerates inference speed and achieves 1000+ sentences per second (80 times QPS). 
    ...
    8 gpu workers can be started and evenly distributed on 4 cards.


<h2 align="center">Introduction of API</h2>
...
Therefore, we have provided the ```ManagedModel``` class to facilitate model lazy initialization and migration while supporting multiple GPU cards.
...
<h2 align="center">BenchmarkTest</h2>
...
We adopt [Future API](#Future API Usage) to conduct multi-GPU benchmeark test locally in order to reduce the influence of performance bottleneck of web server. Please refer to code example in [future_example.py](example/future_example.py)
